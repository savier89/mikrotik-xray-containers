...
test "API server script exists" "$(ls /api_server.py > /dev/null 2>&1 && echo 0 || echo 1)"
test "API server is executable" "$(ls -l /api_server.py > /dev/null 2>&1 && echo 0 || echo 1)"
test "Python3 exists" "$(ls /usr/bin/python3 > /dev/null 2>&1 && echo 0 || echo 1)"
